...
#include <stdlib.h> // using drand48() for random number generation
...
inline void matMult(double A[4][4], double B[4][4])
{
 ...
 // pre-multiplying a given transformation matrix by one of our
...
/**
 * Matrix transpose. The transpose of A is written to U.
 * @param A The matrix to be transposed.
 * @param U [out] transpose of the matrix
 */
inline void matTrans(double A[4][4], double U[4][4])
{
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      U[j][i] = A[i][j];  // Transpose the element at (i, j) to (j, i)
    }
  }
}
...
{
 // Initializes the given ray3D struct with the position
...
/**
 * Perform bi-linear interpolation on the nearest 4 pixels around (x,y) on the mapped image.
 * @param img the image
 * @param x   the horizontal coordinate of pixel
 * @param y   the y coordinate of the pixel (vertical)
 * @param result   [out] the interpolated pixel colour
 */
inline void interpolatePixel(struct image *img, double x, double y, struct colourRGB *result)
{
  ...
  struct colourRGB p00 = {  // Top-left
          .R = ((double *)(img->rgbdata)) [(y0 * img->sx + x0) * 3 + 0],
          .G = ((double *)(img->rgbdata)) [(y0 * img->sx + x0) * 3 + 1],
          .B = ((double *)(img->rgbdata)) [(y0 * img->sx + x0) * 3 + 2]};
  struct colourRGB p10 = {  // Top-right
          .R = ((double *)(img->rgbdata)) [(y0 * img->sx + x1) * 3 + 0],
          .G = ((double *)(img->rgbdata)) [(y0 * img->sx + x1) * 3 + 1],
          .B = ((double *)(img->rgbdata)) [(y0 * img->sx + x1) * 3 + 2]};
  struct colourRGB p01 = {  // Bottom-left
          .R = ((double *)(img->rgbdata)) [(y1 * img->sx + x0) * 3 + 0],
          .G = ((double *)(img->rgbdata)) [(y1 * img->sx + x0) * 3 + 1],
          .B = ((double *)(img->rgbdata)) [(y1 * img->sx + x0) * 3 + 2] };
  struct colourRGB p11 = {  // Bottom-right
          .R = ((double *)(img->rgbdata)) [(y1 * img->sx + x1) * 3 + 0],
          .G = ((double *)(img->rgbdata)) [(y1 * img->sx + x1) * 3 + 1],
          .B = ((double *)(img->rgbdata)) [(y1 * img->sx + x1) * 3 + 2] };

  // Perform bi-linear interpolation for each color channel
  result->R = (
          p00.R * (1 - x_frac) * (1 - y_frac) +
          p10.R * x_frac * (1 - y_frac) +
          p01.R * (1 - x_frac) * y_frac +
          p11.R * x_frac * y_frac
  );

  result->G = (
          p00.G * (1 - x_frac) * (1 - y_frac) +
          p10.G * x_frac * (1 - y_frac) +
          p01.G * (1 - x_frac) * y_frac +
          p11.G * x_frac * y_frac
  );

  result->B = (
          p00.B * (1 - x_frac) * (1 - y_frac) +
          p10.B * x_frac * (1 - y_frac) +
          p01.B * (1 - x_frac) * y_frac +
          p11.B * x_frac * y_frac
  );
}
...
void coneCoordinates(struct object3D *plane, double a, double b, double *x, double *y, double *z);
...
void coneSample(struct object3D *plane, double *x, double *y, double *z);
